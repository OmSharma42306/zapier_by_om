...
interface Action{
    ...
    avilableActionName : string;
}

export default function(){
    const [selectedTrigger,setSelectedTrigger] = useState("");
    const [selectedAction,setSelectedActions]  = useState<Action[]>([]);
    ...
    return <div>
        ...
        {openModalIndex ? <ServicesModal/>: ""}
            <div className="flex justify-center w-full">
            <ZapCell name={selectedTrigger?selectedTrigger:"Trigger"}  index={1}  onClick={() => setOpenModalIndex(1)} />
            </div>

            <div className="  w-full pt-2 pb-2">
            {selectedAction.map((action,index) =><div className=" pt-2 flex justify-center"> <ZapCell index={2+index} name={action.avilableActionName ? action.avilableActionName:"Action"}></ZapCell></div>)}
            </div>
...
function Modal ({index}:{index:number}){
    return <div id="static-modal" data-modal-backdrop="static"   className="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
